...
cla = []
for i, name in enumerate(["airplane", "ambulance", "backpack", "banana", "bicycle", "birthday cake", "book", "bus", "camera", "cat", "cloud", "dog", "helicopter", "house", "octopus", "sailboat", "The Eiffel Tower", "The Great Wall of China", "tree", "violin"]):
    with open("D:\Python/NF_2024\quick_draw\data\exp\simplified/512point_diff_quarter/X/512point_"+name+".pickle", mode='br') as f:
        tmp = pickle.load(f)
        X_train += tmp
        cla += [i]*len(tmp)
    with open("D:\Python/NF_2024\quick_draw\data\exp\simplified/512point_diff_quarter/y/512point_"+name+".pickle", mode='br') as f:
        tmp = pickle.load(f)
        y_train += tmp
X_train, X_test, y_train, y_test = train_test_split(X_train, y_train, test_size=0.2, stratify=cla)
"""
...
model = PointGANGenerator1(num_points, num_points)
...
run = wandb.init(project="QD_pointgan_G_pre")
...
for epoch in range(num_epoch):
    ...
    torch.save(model.state_dict(), "model_weights/pointgan_G_pre3_"+str(epoch+1)+".pth")
